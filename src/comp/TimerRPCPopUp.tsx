...
type RPCPopUp = {
  timeRemaining: Date;
  setShowTimerPopUp: (value: boolean) => void;
};

const TimerRPCPopUp = ({ timeRemaining, setShowTimerPopUp }: RPCPopUp) => {
  const [showBuyingOptions, setShowBuyingOptions] = useAtom(tutorialBuyModal);

  const formatTime = () => {
    const time = new Date(timeRemaining);

    // get the time remaining in milliseconds from the time remanign and now
        ...
        <div className="relative z-10 flex flex-col items-center justify-center text-center">
          ...
          <div className="mt-10 flex flex-row items-center">
            <div className="mx-2 flex h-[100px] w-[84px] flex-col items-center justify-center rounded-xl bg-[#FAFAFA]">
              {" "}
              <p className="text-[26px] font-bold">{formatTime().hours}</p>{" "}
