...
async function createSignatureMessage(inputIndex: number, version: string, inputCountLE: string, inputs: TxInput[], outputCountLE: string, outputs: TxOutput[], locktime: string, sighashFlag: string) : Promise<string> {
  ...
  for(let i = 0; i < inputs.length; i++) {
    ...
    if(i == inputIndex) {
      ...
      console.log("selected vout in createSignatureMessage LE: " + inputs[i].vout);
      console.log("selected txid in createSignatureMessage number: " + parseInt(inputs[i].vout, 16));
  ...
  //console.log("hashedOnce: " + CryptoJS.enc.Hex.parse(prehashedMessage));
  console.log("hashedMessage: " + CryptoJS.SHA256(CryptoJS.SHA256(CryptoJS.enc.Hex.parse(prehashedMessage))));
